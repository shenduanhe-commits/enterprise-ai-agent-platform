...
| R1 | JWT access + refresh；资源按 `user_id` 隔离；HTTPS 仅生产再谈（机制见 [HTTPS.md](../03-development/HTTPS.md)） |
| R2 | 危险工具 HITL；工具超时 |
...
- 本地 HTTP 可接受；演示不要暴露到公网。HTTPS 加密了什么、私钥干什么、地址会不会被看见：见 [HTTPS.md](../03-development/HTTPS.md)。
- 密钥：`.env`，不入库。
- 密码：只存哈希。
- 会话内容视为企业数据，列表/详情必须鉴权（R1）。
- Access 走 `Authorization`；refresh 只出现在 `/auth/refresh` 的 JSON，核心是让长票少露面。当前每次刷新会换新的 7 天 refresh（滑动续期）。两枚都塞进 localStorage 时 XSS 可以一起偷走。Cookie 只带给种它的主机，不会随请求寄到任意网站。详见 [JWT.md](../03-development/JWT.md) 第 9–10 节。
- CORS 不是接口验身份：预检和 `Allow-Origin` 只对浏览器有用；curl 不带 Origin 也会通。见 [CORS.md](../03-development/CORS.md)。
