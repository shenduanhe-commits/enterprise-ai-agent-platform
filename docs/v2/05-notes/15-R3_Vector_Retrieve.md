...
\[
\mathrm{score} = \mathrm{dense\_score} + 2 \times \frac{\text{问句词有多少个出现在 chunk 原文里}}{\text{问句词总数}}
\]
...
| --- | --- | --- |
| 公式 | \(\sum_{i \in q \cap d} q_i \cdot d_i\)（Qdrant 再乘 IDF） | \(\dfrac{\text{问句词有几个作为子串出现}}{\text{问句词总数}}\) |
